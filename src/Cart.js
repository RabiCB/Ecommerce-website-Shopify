...
*/
  return (
    ...
    <div className="h-16 relative bg-black text-white flex pr-8 pl-8 items-center justify-between">
      ...
      {!isEmpty ? (
        <>
          <div className="grid grid-cols-4 max-sm:grid-cols-1 max-sm:grid-rows-2  m-4 grid-rows-1 gap-5 max-lg:grid-cols-2 max-lg:grid-rows-4">
            {items.map((y) => {
        
              return (
                
                <div key={y.id}>
                  <div className="bg-cyan max-md:h-30 max-md:w-30 h-66 w-45 rounded-md border-2 flex gap-2  flex-col">
                    <div className=" flex justify-center items-center">
                      <img
                        className="w-30 h-60 object-contain rounded p-3  "
                        src={y.img}
                        alt="bag"
                      />
                    </div>
